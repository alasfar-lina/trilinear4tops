...
def runMCMC4(likelihood, limits, trace_dir='', config=[], fit=True):
    ...
    with pm.Model() as model:
        ...
        k5 = pm.Uniform('k5', lower=limits[8], upper=limits[9])

        like = pm.Potential('like', likelihood(k1, k2, k3, k4,k5))
...
def mainquad():
    llCqtm =lambda Cqu1,Cqu8,Cquqbp,Cqq3,CH :mylikelihoodAV(Cqu1,Cqu8,
                                                   1/2/(2*Nc+1)*Cquqbp,+1/CF*Cquqbp/2,Cqq3,CH,
                                                   data,collider='Run-II',mode='rge',l3mode='quadratic')
    limits = [-12., 15,-30.0,60.,-6.,6.0,-200,200, -90, 50] #for lin run
...
def mainlin():
    llCqtm =lambda Cqu1,Cqu8,Cquqbp,Cqq3,CH :mylikelihoodAV(Cqu1,Cqu8,
                                                   1/2/(2*Nc+1)*Cquqbp,+1/CF*Cquqbp/2,Cqq3,CH,
                                                   data,collider='Run-II',mode='rge',l3mode='linear')
    #limits = [-8., 12,-25.0,40.,-3.,5.0, -35, 25] #for quad run2
    limits = [-12., 15,-30.0,60.,-6.,6.0,-200,200, -90, 50] #for lin run2
    #limits = [-4., 4.,-15.,15.,-4.,4., -30, 30]
    config = [200000, 0.8, 50]
    trace_dir='../results/fits/5paramfit_LHC_RunII_l3L_rge.pickle'
    trace_1, model_1 = runMCMC4(llCqtm, limits, config=config,trace_dir=trace_dir)    
