...
import pandas as pd
from pyik.mplext import lighten_color
try:
...
def plotGTC(chains, **kwargs):
    ...
    def_pal =sns.diverging_palette(220, 20, n=201)
    def_palh =def_pal.as_hex()
    ...
    colorsDict = {
                 ...
                 'lina_sp':('#7395ab','#aebdcb','#cdd7e2'),
                  'lina_blues':('#9cadce','#cad0e4','#dbe0ee'),
    ...
    #colorsOrder = kwargs.pop('colors', defaultColorsOrder)
   ...
   # if __isstr(colorsOrder):
    ...
    colors =  kwargs.pop('colors', '#7395ab') #[colorsDict[cs] for cs in colorsOrder]
    usepal =  kwargs.pop('usepalette', True)
    ...
    colpal = (kwargs.pop('palette', def_palh)).as_hex()
    ...
    bildgruss = kwargs.pop('Bildgruess',(10,10))
    ...
    # Define colormap
    myColorMap = sns.diverging_palette(220, 20, n=201,as_cmap=True)#setCustomColorMaps(['#7395ab','#aebdcb','#cdd7e2'])
    ...
    fig = plt.figure(figsize=bildgruss)
            ...
            for j in range(nDim):  # column
                if j < i:
                    ...
                    ax = __plot2d(ax, nChains, chainsForPlot2D, weights, nBins,
                                  smoothingKernel, filledPlots, colors,colpal,usepal,
    ...
    if do1dPlots:
        ...
        for i in range(nDim):
            ...
            if usepal==True:
                          ...
                          smoothingKernel, filledPlots, colpal[0],
...
def __plot2d(ax, nChains, chains2d, weights, nBins, smoothingKernel,
             filledPlots, colors,colpal,usepal, nContourLevels, confLevels, truths2d,
    ...
    for k in reversed(range(nChains)):
        samples = np.vstack((chains2d[k][0], chains2d[k][1])).T
        tr = pd.DataFrame(samples)
        corr=tr.corr()
        #print(corr)
        corr=(corr.to_numpy())[1,0]
        # Is there a chain to plot?
        if (np.isnan(chains2d[k][0]).all()) | (np.isnan(chains2d[k][1]).all()):
            ...
            if filledPlots:
                ...
                if usepal==True:
                    col = colpal[int(np.floor(100-(100*corr)))]
                    lev=chainLevels[k]
                   #print(lev.shape)
                    for i in range(nContourLevels):
                        ax.contourf(xbins, ybins, plotData[-1], levels=[lev[i],lev[i+1]],
                        colors=col, alpha=1/nContourLevels+i/nContourLevels )
                    ...
                    for i in range(nContourLevels):
                        ax.contourf(xbins, ybins, plotData[-1], levels=[lev[i],lev[i+1]],
                            colors=colors, alpha=1/nContourLevels+i/nContourLevels )
        ...
        if plotData[nChains-1-k] is not None:
            for l in range(nContourLevels):
                ax.contour(plotData[nChains-1-k],
                           ...
                           linewidths=1, colors='#6B6B75',linestyles=ls[l]
                          # colors=colors[k][l]
                          )
