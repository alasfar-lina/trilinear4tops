...
def mylikelihood(operator,Cqu1,CH,data,experiments=['ATLAS','CMS'],HiggsChannels=['ggf','vbf','ttxh','vh'],TopChannels='smeft_fit_quad',linearl3=True,linearmu=True):
    ...
    gamma_gaga_cqu1= data[operator]['gagaos']* Cqu1
    gamma_gg_cqu1=data[operator]['ggFos']* Cqu1
    gamma_bb_cqu1=data[operator]['Hbb']* Cqu1
    sigma_gg_cqu1 =gamma_gg_cqu1
    sigma_ttH_cqu1= data[operator]['ttH']* Cqu1
    cH=CH
...
###########################################################################
   ...
   (delta_ch_vbf**2+delta_ch_ww**2)**.5,\
    ...
    errtthTheo = np.array([(delta_ch_tth**2+delta_ch_gaga**2+data[operator]['ttH_delta']**2)**.5,\
    (delta_ch_tth**2+delta_ch_vhdecay**2+data[operator]['ttH_delta']**2)**.5,\
                           0.,\
    (delta_ch_tth**2+data[operator]['ttH_delta']**2)**.5,\
    (delta_ch_tth**2+data[operator]['ttH_delta']**2)**.5\
        ])
    errtthTheocms = np.array([(delta_ch_tth**2+delta_ch_gaga**2+data[operator]['ttH_delta']**2)**.5,\
    (delta_ch_tth**2+delta_ch_vhdecay**2+data[operator]['ttH_delta']**2)**.5,\
                           0.,0.,0.,\
        ])
    muth1={
    ...
    ##include the prior
    #chi2d=chi2d-0.5*np.dot(deltamu**2,covp)
    return chi2d
